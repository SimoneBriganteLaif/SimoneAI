...
<task type="auto">
  ...
  <action>
    ...
    **7. Wire link click and relationship row click** in init():
    ...
    // Relationship row click -> same popup (per CONTEXT.md: both link click AND row click open same popup)
    document.addEventListener('click', function(evt) {
        ...
        // Traverse up to find the JointJS element
        var jointEl = row.closest('.joint-element');
        if (!jointEl) return;

        // Find the JointJS model element from the DOM element
        var graph = ERCanvas.getGraph();
        var paper = ERCanvas.getPaper();
        var elements = graph.getElements();
        var element = null;
        ...
        // Determine source/target relationship pair
        // The clicked element might be the source or target side of the relationship
        ...
        // Check if the clicked table has the FK (child side) or not (parent side)
        var hasFkToTarget = tableData.columns.find(function(c) { return c.foreign_key && c.foreign_key.includes(targetData.table_name); });
        ...
        if (hasFkToTarget) {
            ...
            fkCol = tgtData.columns.find(function(c) { return c.foreign_key && c.foreign_key.includes(srcData.table_name); });
        }

        // Find the JointJS link for this relationship
  ...
  <acceptance_criteria>
    ...
    - editor.js contains `er-rel-row[data-rel-name]` (click on relationship row opens same edit popup)
    - editor.js relationship row click handler has full implementation (finds element, finds counterpart table, calls showRelationshipEditPopup)
  </acceptance_criteria>
