...
descrizione: >
  ...
  Coordina 4 sub-skill con gate di qualità tra le fasi.
  Può essere eseguito end-to-end o ripreso da una fase specifica.
  NON è l'unico modo per sviluppare — le sub-skill funzionano anche standalone.
fase: development
versione: "1.0"
stato: beta
legge:
...
Orchestra il ciclo di vita completo di una feature attraverso 4 fasi con gate di qualità:
**Plan → Develop → Test + Review → Exit**.

Coordina le sub-skill (`feature-plan`, `feature-develop`, `feature-test`, `feature-review`) e gestisce i gate tra le fasi.

---

## Perimetro

**Fa**: orchestrazione delle 4 fasi, gestione gate, stato condiviso via `.feature-state.md`, aggiornamento `feature-log.md` a fine ciclo, collegamento con skill esistenti (`estrazione-decisioni`, `verifica-pre-commit`).
...
- Quando si vuole riprendere una feature in corso (`.feature-state.md` esiste già)
- Per feature di complessità media o grande (per task piccoli, le sub-skill standalone sono sufficienti)
...
3. **Riprendere da una fase precedente?** (solo se `.feature-state.md` esiste già — mostra la fase corrente e chiedi se riprendere o ricominciare)
...
3. Lo sviluppo viene registrato nella sezione `## Sviluppo` di `.feature-state.md`
...
- [ ] Conferma dell'utente che lo sviluppo è completo
...
1. **Decisioni tecniche**: se durante lo sviluppo sono emerse decisioni non banali:
   ...
   - Se sì → invoca `skills/development/estrazione-decisioni/`
2. **Feature log**: aggiorna `projects/[nome]/feature-log.md` con una nuova entry:
...
3. **Pattern candidati**: se la review ha identificato pattern estraibili:
...
4. **Cleanup**: aggiorna `.feature-state.md` stato → `completata`, poi eliminalo
5. **Commit**: procedi con il commit (→ scatta `verifica-pre-commit` come da Regola 1 di CLAUDE.md)

---

## Stato condiviso: `.feature-state.md`

File temporaneo che tiene traccia del progresso. Prefisso `.` per segnalare che è transitorio. Eliminato a fine workflow.
...
- [ ] `.feature-state.md` è stato eliminato
- [ ] `feature-log.md` è stato aggiornato
