...
| IDEA-015 | Rivedere gestione logging su progetti LAIF | processo | M | media | proposta | 2026-03-10 | I log su CloudWatch sono poco parlanti e difficili da debuggare (es. mancano dettagli su errori OAuth callback). Definire standard di logging strutturato (livelli, contesto request, stack trace) da applicare a laif-template e progetti derivati |
| IDEA-016 | Creare `knowledge/team/` con profili persone LAIF per assegnazioni task automatiche | struttura | M | bassa | proposta | 2026-03-12 | La skill `crea-task-notion` propone task ma non assegna persone. Creare profili in `knowledge/team/` (nome, ruolo, competenze, progetto primario) per permettere in futuro assegnazioni automatiche dei task basate su competenze e disponibilità. Prerequisito: usare `crea-task-notion` su più progetti per capire i pattern di assegnazione reali |
| IDEA-017 | Scheduled task: esecuzione periodica `sistema-riunioni-notion` in dry-run con report | automazione | S | media | proposta | 2026-04-22 | Ogni 1-2 settimane lanciare la skill in modalità dry-run e presentare un report delle anomalie (pagine senza Tag, senza icona, senza Progetto, titoli placeholder). L'utente approva e si applica. Prerequisito: aggiungere modalità `--dry-run` alla skill. Candidato per scheduled task di Claude Code |
