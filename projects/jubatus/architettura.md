...
```mermaid
graph TB
    Browser["Browser<br/>(Operatori CS)"]
    NextJS["Next.js 16<br/>App Router<br/>Port 8080"]
    ...
    subgraph Backend["FastAPI Modules"]
        Template["Template modules<br/>Auth, users, chat,<br/>files, notifications"]
        App["App modules<br/>Email, changelog"]
    end

    FastAPI --- Template
    FastAPI --- App

    PostgreSQL["PostgreSQL 15<br/>(applicativo)<br/>Port 5432"]
    MySQL["MySQL RDS<br/>(read-only)<br/>Jubatus AWS"]
    S3["AWS S3<br/>(foto/video)<br/>Jubatus AWS"]
    ...
    subgraph Providers["Email Providers"]
        Gmail["Gmail<br/>OAuth"]
        MS["Microsoft<br/>Graph"]
        SES["AWS SES<br/>(invio)"]
    end
